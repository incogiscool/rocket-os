    ...
    switch (minmea_sentence_id(line, false)) {
        case MINMEA_SENTENCE_RMC: {
            ...
            if (minmea_parse_rmc(&frame, line)) {
                ESP_LOGI("TEST: ", INDENT_SPACES "$xxRMC: raw coordinates and speed: (%d/%d,%d/%d) %d/%d\n",
                        frame.latitude.value, frame.latitude.scale,
                        frame.longitude.value, frame.longitude.scale,
                        frame.speed.value, frame.speed.scale);
                ESP_LOGI("TEST: ", INDENT_SPACES "$xxRMC fixed-point coordinates and speed scaled to three decimal places: (%d,%d) %d\n",
                        minmea_rescale(&frame.latitude, 1000),
                        minmea_rescale(&frame.longitude, 1000),
                        minmea_rescale(&frame.speed, 1000));
                ESP_LOGI("TEST: ", INDENT_SPACES "$xxRMC floating point degree coordinates and speed: (%f,%f) %f\n",
                        minmea_tocoord(&frame.latitude),
                        minmea_tocoord(&frame.longitude),
                        minmea_tofloat(&frame.speed));
            }
            else {
                ESP_LOGI("TEST: ", INDENT_SPACES "$xxRMC sentence is not parsed\n");
            }
        } break;
        ...
        case MINMEA_SENTENCE_GGA: {
            ...
            if (minmea_parse_gga(&frame, line)) {
                ESP_LOGI("TEST: ", INDENT_SPACES "$xxGGA: fix quality: %d\n", frame.fix_quality);
            }
            else {
                ESP_LOGI("TEST: ", INDENT_SPACES "$xxGGA sentence is not parsed\n");
            }
        } break;
        ...
        case MINMEA_SENTENCE_GST: {
            ...
            if (minmea_parse_gst(&frame, line)) {
                ESP_LOGI("TEST: ", INDENT_SPACES "$xxGST: raw latitude,longitude and altitude error deviation: (%d/%d,%d/%d,%d/%d)\n",
                        frame.latitude_error_deviation.value, frame.latitude_error_deviation.scale,
                        frame.longitude_error_deviation.value, frame.longitude_error_deviation.scale,
                        frame.altitude_error_deviation.value, frame.altitude_error_deviation.scale);
                ESP_LOGI("TEST: ", INDENT_SPACES "$xxGST fixed point latitude,longitude and altitude error deviation"
                        " scaled to one decimal place: (%d,%d,%d)\n",
                        minmea_rescale(&frame.latitude_error_deviation, 10),
                        minmea_rescale(&frame.longitude_error_deviation, 10),
                        minmea_rescale(&frame.altitude_error_deviation, 10));
                ESP_LOGI("TEST: ", INDENT_SPACES "$xxGST floating point degree latitude, longitude and altitude error deviation: (%f,%f,%f)",
                        minmea_tofloat(&frame.latitude_error_deviation),
                        minmea_tofloat(&frame.longitude_error_deviation),
                        minmea_tofloat(&frame.altitude_error_deviation));
            }
            else {
                ESP_LOGI("TEST: ", INDENT_SPACES "$xxGST sentence is not parsed\n");
            }
        } break;
        ...
        case MINMEA_SENTENCE_GSV: {
            ...
            if (minmea_parse_gsv(&frame, line)) {
                ESP_LOGI("TEST: ", INDENT_SPACES "$xxGSV: message %d of %d\n", frame.msg_nr, frame.total_msgs);
                ESP_LOGI("TEST: ", INDENT_SPACES "$xxGSV: satellites in view: %d\n", frame.total_sats);
                for (int i = 0; i < 4; i++)
                    ESP_LOGI("TEST: ", INDENT_SPACES "$xxGSV: sat nr %d, elevation: %d, azimuth: %d, snr: %f dbm\n",
                        frame.sats[i].nr,
                ...
                ESP_LOGI("TEST: ", INDENT_SPACES "$xxGSV sentence is not parsed\n");
            }
        } break;
        ...
        case MINMEA_SENTENCE_VTG: {
            ...
            if (minmea_parse_vtg(&frame, line)) {
                ESP_LOGI("TEST: ", INDENT_SPACES "$xxVTG: true track degrees = %f\n",
                        minmea_tofloat(&frame.true_track_degrees));
                ESP_LOGI("TEST: ", INDENT_SPACES "        magnetic track degrees = %f\n",
                        minmea_tofloat(&frame.magnetic_track_degrees));
                ESP_LOGI("TEST: ", INDENT_SPACES "        speed knots = %f\n",
                        minmea_tofloat(&frame.speed_knots));
                ESP_LOGI("TEST: ", INDENT_SPACES "        speed kph = %f\n",
                        minmea_tofloat(&frame.speed_kph));
            }
            else {
                ESP_LOGI("TEST: ", INDENT_SPACES "$xxVTG sentence is not parsed\n");
            }
        } break;
        ...
        case MINMEA_SENTENCE_ZDA: {
            ...
            if (minmea_parse_zda(&frame, line)) {
                ESP_LOGI("TEST: ", INDENT_SPACES "$xxZDA: %d:%d:%d %02d.%02d.%d UTC%+03d:%02d\n",
                        frame.time.hours,
                        frame.time.minutes,
                        frame.time.seconds,
                        frame.date.day,
                ...
                ESP_LOGI("TEST: ", INDENT_SPACES "$xxZDA sentence is not parsed\n");
            }
        } break;

        case MINMEA_INVALID: {
            ESP_LOGI("TEST: ", INDENT_SPACES "$xxxxx sentence is not valid\n");
        } break;

        default: {
            ESP_LOGI("TEST: ", INDENT_SPACES "$xxxxx sentence is not parsed\n");
        } break;
    }
}
